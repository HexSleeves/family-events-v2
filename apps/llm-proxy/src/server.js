...
const UPSTREAM_BASE_URL = normalizeBaseUrl(process.env.LLM_UPSTREAM_BASE_URL ?? "")
...
async function proxy(req) {
  ...
  if (url.pathname === "/healthz") {
    return json(200, { ok: true, upstream_configured: Boolean(UPSTREAM_BASE_URL) })
  }
  ...
  import("node:http").then(({ createServer }) => {
    createServer(async (nodeReq, nodeRes) => {
      ...
      const req = new Request(new URL(nodeReq.url ?? "/", origin), {
        ...
        headers,
        body: nodeReq.method === "GET" || nodeReq.method === "HEAD" ? undefined : nodeReq,
        duplex: "half",
      })

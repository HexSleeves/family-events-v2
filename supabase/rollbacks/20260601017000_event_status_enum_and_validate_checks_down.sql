-- Rollback for 20260601017000_event_status_enum_and_validate_checks.sql
-- Reverts public.events.status from public.event_status back to text, restores
-- the text-based dependents/functions, and re-adds the six data-integrity
-- constraints as NOT VALID (undoing the VALIDATE).

BEGIN;

-- 1. Drop enum-era dependents.
...
-- 2. Revert column type to text and restore the CHECK.
...
ALTER TABLE public.events
  ...
  CHECK (status = ANY (ARRAY['draft'::text, 'published'::text, 'rejected'::text, 'archived'::text]));

-- 3. Recreate text-predicate dependents.
CREATE INDEX events_local_date_published_idx ON public.events
  USING btree ((((start_datetime AT TIME ZONE timezone))::date)) WHERE (status = 'published'::text);
CREATE INDEX events_published_city_start_datetime_idx ON public.events
  USING btree (city_id, start_datetime) WHERE (status = 'published'::text);
CREATE INDEX events_published_feed_idx ON public.events
  USING btree (city_id, start_datetime, id) WHERE (status = 'published'::text);
CREATE INDEX events_published_local_date_city_idx ON public.events
  USING btree ((((start_datetime AT TIME ZONE timezone))::date), city_id, id) WHERE (status = 'published'::text);
CREATE INDEX events_published_start_id_idx ON public.events
  USING btree (start_datetime, id) WHERE (status = 'published'::text);
...
CREATE POLICY "Authenticated reads published or admin reads all" ON public.events
  ...
  USING ((( SELECT private.is_admin() AS is_admin) OR (status = 'published'::text)));
...
-- 4. Restore the original (text-comparison) function bodies.
...
-- 5. Re-add the six data-integrity constraints as NOT VALID (undo VALIDATE).
ALTER TABLE public.events DROP CONSTRAINT IF EXISTS events_age_range_chk;
ALTER TABLE public.events
...
ALTER TABLE public.events DROP CONSTRAINT IF EXISTS events_lat_lng_chk;
ALTER TABLE public.events
...
ALTER TABLE public.events DROP CONSTRAINT IF EXISTS events_price_chk;
ALTER TABLE public.events
  ADD CONSTRAINT events_price_chk CHECK (price IS NULL OR price >= 0) NOT VALID;
ALTER TABLE public.user_profiles DROP CONSTRAINT IF EXISTS user_profiles_child_age_chk;
ALTER TABLE public.user_profiles
  ADD CONSTRAINT user_profiles_child_age_chk
  CHECK (child_age IS NULL OR child_age BETWEEN 0 AND 18) NOT VALID;
ALTER TABLE public.invite_codes DROP CONSTRAINT IF EXISTS invite_codes_used_count_max_chk;
ALTER TABLE public.invite_codes
  ADD CONSTRAINT invite_codes_used_count_max_chk CHECK (used_count <= max_uses) NOT VALID;
ALTER TABLE public.event_sources DROP CONSTRAINT IF EXISTS event_sources_scrape_interval_chk;

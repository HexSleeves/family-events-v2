...
export function mapMacaroniKidEvent(raw: unknown, sourceBase: string): ParsedEvent | null {
  ...
  const startDatetime = parseIsoDate(asString(node.start) ?? asString(node.startDate))
  if (!startDatetime) return null
  const endDatetime = parseIsoDate(asString(node.end) ?? asString(node.endDate))
...
export const macaroniKidParser: SourceParser<"macaronikid"> = {
  type: "macaronikid",
  fetchAndParse(source, ctx) {
    return fetchMacaroniKidEvents(source, ctx.fetchText, ctx.fetchJson)
  },
}

...
export async function findFallbackImage(
  ...
  for (const { searchTerm } of queue) {
    // Two-pass strategy: try the bare term first (e.g. "splash park"), then
    // fallback to " family" suffix only if the bare query returns empty.
    // This keeps activity-specific queries sharp while still supporting
    // generic family events when needed.
    ...
    for (const query of attempts) {
      ...
      try {
        ...
        // Network / timeout / parse failure: try the next pass, then bail.
        continue
      }
    }
  }

...
function normalizeAiTrace(trace: EventAiTrace | null): EventAiTraceWithParsed | null {
  if (!trace) {
    return null
  }
...
export function useAdminEventAiTrace(eventId: string | null) {
  return useQuery({
    ...
    queryFn: async (): Promise<EventAiTraceWithParsed | null> => {
      if (!eventId) {
        return null
      }
...
export function useUpdateAdminEventTags() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      void queryClient.invalidateQueries({ queryKey: ["admin", "events"] })
      void queryClient.invalidateQueries({ queryKey: ["admin", "event-ai-trace", variables.eventId] })
      void queryClient.invalidateQueries({ queryKey: ["events"] })
      void queryClient.invalidateQueries({ queryKey: ["event"] })
    },
  })

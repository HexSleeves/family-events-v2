  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          <p className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-1">Dashboard</p>
          <h1 className="text-3xl font-extrabold text-foreground tracking-tight text-balance">{greeting}</h1>
          {profile?.child_name && (
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-foreground flex items-center gap-2">
            ...
            {user && profile?.child_name ? `Recommended for ${profile.child_name}` : "Happening Near You"}
          </h2>
          <Button variant="ghost" size="sm" className="text-primary gap-1" asChild>
            <Link to="/explore">
              See all <ArrowRight className="h-3 w-3" />
      ...
      {savedEvents.length > 0 && (
        <section>
          ...
          <Button variant="outline" className="w-full mt-3 border-primary text-primary hover:bg-primary hover:text-primary-foreground" asChild>
            <Link to="/saved">
              Explore All Saved ({favorited.size})
            </Link>
          </Button>
        </section>
      )}
      ...
      <div className="rounded-2xl bg-primary p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-primary-foreground/80 text-xs mt-0.5 leading-relaxed">
            Weekend sensory play sessions are usually less crowded after 11:15 AM. Early arrival recommended for most popular events!
          </p>
        </div>
      </div>
